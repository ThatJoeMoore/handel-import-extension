...
import { S3Service } from '../src/service';
import accountConfig from './fake-account-config';

describe('s3 service deployer', () => {
    ...
    const serviceProvisioner = new S3Service();

    beforeEach(async () => {
        serviceParams = {
            type: serviceType
        };
        serviceContext = new ServiceContext(appName, envName, serviceName, new ServiceType('handel-import-extension', serviceType), serviceParams, accountConfig);
